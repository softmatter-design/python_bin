...
class InitialSetup:
	...
	def calc_conditions(self):
		...
		if self.nw_type == "KG_entangled":
			self.multi_nw = round(self.target_density/init_dens)
			density = self.multi_nw*n_beads_unit/a_cell**3					# 密度
			err_dens = (density/self.target_density - 1)*100
			system = a_cell*self.n_cell										# システムサイズ
			total_beads = self.multi_nw * n_beads_unit * self.n_cell**3		# 総セグメント数
			nu = self.multi_nw*self.n_strand/a_cell**3						# ストランドの数密度
			text += "設定密度:\t\t\t" + str(self.target_density) + "\n"
			text += "多重度:\t\t\t\t" + str(self.multi_nw) + "\n"
			...
			nu = self.multi_nw*self.n_strand*self.n_cell**3/vol							# ストランドの数密度
		...
		#
		n_solvent = 0
		target_cond = [system, a_cell, total_beads, nu, structure, self.multi_nw, n_solvent]
		return target_cond
	...
	def set_target_name(self):
		target_name = str(self.nw_type ) + "_" + str(self.n_strand) + '_Chains_N_' + str(self.n_segments) + "_C_" + str(self.n_cell) + "_M_" + str(self.multi_nw)
		return target_name
