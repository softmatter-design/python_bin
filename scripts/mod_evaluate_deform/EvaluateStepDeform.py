...
def script_content(f_data):
	...
	if f_data == 'ss_step.dat':
		...
		script += 'set label 1 sprintf("fitting region: from %.d", s) at graph 0.4, 0.5\n'
		script += 'set label 2 sprintf("G_{eq} = %.1e", eq) at graph 0.4, 0.4\n'
		script += 'set label 3 sprintf("{/Symbol t} = %.1e", tau) at graph 0.4, 0.3\n'
		script += 'set label 5 sprintf("{/Symbol n}k_BT = %.2e", G) at graph 0.4, 0.6\n\n'
		script += 'set xlabel "Time"\nset ylabel "G(t)"\n'	
		script += 'set logscale xy\n\n'	
		script += 'set format x "10^{%L}"\nset format y "10^{%L}"\n'
		script += 'plot	data u 1:2 axis x1y1 w l lw 2 lt 1 ti "G(t)", \\\n'
